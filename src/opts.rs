...
fn to_config(matches: ArgMatches) -> Config {
    ...
    let source_file_string = matches.value_of("source_file").expect("I need a source file to work with.");
    let source_file = PathBuf::from(source_file_string.to_owned());
    let target_file = match matches.is_present("target_file") {
        true => {
            let str = matches.value_of("target_file").expect("You will have to specify a target file.");
